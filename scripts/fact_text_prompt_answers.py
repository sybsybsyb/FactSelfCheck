...
from hallucinations_kg.models.prompt_predictor import FactTextPromptAgent
...
@hydra.main(
    version_base="1.3", config_path=str(ROOT_PATH / "config"), config_name="predict_text_prompt"
)
...
def add_prompt_answers(
    ...
    samples = entry[samples_column]

    predictions = []
    for g in sentences_graphs:
        sentence_predictions = [agent.get_answers(fact, samples) for fact in g.triples]
        predictions.append(sentence_predictions)
    entry["fact_text_prompt_answers"] = predictions
    return entry


def get_agent(cfg: DictConfig) -> FactTextPromptAgent:
    ...
    return FactTextPromptAgent(prompt=prompt, llm=llm)
